...
void Calculator::GetPostfix() {
    ...
    for (cur_pos_ = infix_.begin(); cur_pos_ != infix_.end() && good_to_go_; cur_pos_++) {
        if (*cur_pos_ == 'x') {
...
bool Calculator::IsValidFunc(const char check) {
    // перепиши и используй итератор
    std::string funcs = "cstal";
    return (funcs.find(check) != std::string::npos);
}

bool Calculator::IsValidFunc() {
    if (*cur_pos_ == 'c' && *(cur_pos_ + 1) == 'o' && *(cur_pos_ + 2) == 's') {
        op_ = COS;
        cur_pos_ += 2;
    } else if (*cur_pos_ == 's' && *(cur_pos_ + 1) == 'i' && *(cur_pos_ + 2) == 'n') {
        op_ = SIN;
        cur_pos_ += 2;
    } else if (*cur_pos_ == 't' && *(cur_pos_ + 1) == 'a' && *(cur_pos_ + 2) == 'n') {
        op_ = TAN;
        cur_pos_ += 2;
    } else if (*cur_pos_ == 'a' && *(cur_pos_ + 1) == 'c' && *(cur_pos_ + 2) == 'o' && *(cur_pos_ + 3) == 's') {
        op_ = ACOS;
        cur_pos_ += 3;
    } else if (*cur_pos_ == 'a' && *(cur_pos_ + 1) == 's' && *(cur_pos_ + 2) == 'i' && *(cur_pos_ + 3) == 'n') {
        op_ = ASIN;
        cur_pos_ += 3;
    } else if (*cur_pos_ == 'a' && *(cur_pos_ + 1) == 't' && *(cur_pos_ + 2) == 'a' && *(cur_pos_ + 3) == 'n') {
        op_ = ATAN;
        cur_pos_ += 3;
    } else if (*cur_pos_ == 's' && *(cur_pos_ + 1) == 'q' && *(cur_pos_ + 2) == 'r' && *(cur_pos_ + 3) == 't') {
        op_ = ATAN;
        cur_pos_ += 3;
    } else if (*cur_pos_ == 'l' && *(cur_pos_ + 1) == 'n') {
        op_ = LN;
        cur_pos_ += 1;
    } else if (*cur_pos_ == 'l' && *(cur_pos_ + 1) == 'o' && *(cur_pos_ + 2) == 'g') {
        op_ = LOG;
        cur_pos_ += 2;
    } else {
        return false;
