...
void Credit::on_pushButton_calculate_clicked() {
  ...
  if (mode == 0) {
    ...
    ui->lineEdit_month->setText(QString("%L1").arg(credit_.GetMonthPayment(), 0, 'f', 2));
    ui->lineEdit_over->setText(QString("%L1").arg(credit_.GetOverpayment(), 0, 'f', 2));
    ui->lineEdit_total->setText(QString("%L1").arg(credit_.GetTotalPayment(), 0, 'f', 2));
    ...
    // double last = payment;
    ui->lineEdit_month->setText(QString("%L1").arg(credit_.GetFirstPayment(), 0, 'f', 2) + ".." +
